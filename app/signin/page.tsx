    ...
    <main className="min-h-screen font-sans relative overflow-hidden bg-gradient-to-br from-indigo-100 via-blue-50 to-purple-100 flex items-center justify-center p-6 md:p-10 text-slate-900">
      ...
      <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl w-full max-w-5xl border border-white/80 overflow-hidden relative z-10 grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 p-8 md:p-12 flex flex-col justify-center" role="main">
          <h3 className="text-2xl font-black text-slate-900 mb-6" tabIndex={0}>
            {isRegisterMode ? 'Daftar Akun Baru' : 'Masuk ke Akun'}
          </h3>

          <form onSubmit={(e) => handleSubmit(e)} className="space-y-4" aria-live="polite">
            {errorMsg && <div role="alert" className="bg-red-50 text-red-700 px-4 py-3 rounded-2xl text-sm font-bold">{errorMsg}</div>}
            {successMsg && <div role="alert" className="bg-emerald-50 text-emerald-700 px-4 py-3 rounded-2xl text-sm font-bold">{successMsg}</div>}

            <div>
              <label htmlFor="username" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">Nama Pengguna</label>
              <input id="username" aria-required="true" aria-label="Masukkan Nama Pengguna" className="w-full border-2 border-slate-200 bg-slate-50/50 p-3.5 rounded-2xl focus:bg-white focus:border-indigo-600 outline-none text-slate-900 font-medium text-sm" type="text" value={username} onChange={(e) => setUsername(e.target.value)} />
            </div>

            <div>
              <label htmlFor="password" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">Kata Sandi</label>
              <input id="password" aria-required="true" aria-label="Masukkan Kata Sandi" className="w-full border-2 border-slate-200 bg-slate-50/50 p-3.5 rounded-2xl focus:bg-white focus:border-indigo-600 outline-none text-slate-900 font-medium text-sm" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>

            <div>
              <label htmlFor="disabilitas" className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">Kebutuhan Aksesibilitas</label>
              <select id="disabilitas" aria-label="Pilih Mode Aksesibilitas" className="w-full border-2 border-slate-200 bg-slate-50/50 p-3.5 rounded-2xl focus:border-indigo-600 outline-none text-slate-900 font-bold text-sm" value={jenisDisabilitas} onChange={(e) => setJenisDisabilitas(e.target.value)}>
                <option value="Tunanetra">Tunanetra (Navigasi Suara)</option>
                <option value="Tunarungu">Tunarungu (Visual & Isyarat)</option>
              </select>
            </div>

            <button type="submit" disabled={isLoading} aria-busy={isLoading} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 rounded-2xl font-bold hover:shadow-lg transition-all mt-2 flex items-center justify-center gap-2">
              {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Memproses...</> : <span>{isRegisterMode ? 'Daftar' : 'Masuk'}</span>}
            </button>
          </form>

          <div className="text-center mt-6 pt-6 border-t border-slate-100">
            <button onClick={() => setIsRegisterMode(!isRegisterMode)} aria-label="Ganti mode pendaftaran atau login" className="text-indigo-600 font-bold text-sm hover:underline">
              {isRegisterMode ? 'Sudah memiliki akun? Masuk' : 'Belum memiliki akun? Daftar'}
            </button>
          </div>

          <div className="mt-8 bg-slate-50 p-4 rounded-2xl border border-slate-200/80" aria-label="Akses Demo Penguji">
            <p className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 text-center mb-3">⚡ Akses Cepat Penguji</p>
            <div className="grid grid-cols-2 gap-2.5">
              <Link href="/tunanetra" className="flex items-center justify-center gap-1.5 bg-white text-blue-700 border border-slate-200 py-2.5 rounded-xl text-xs font-bold shadow-sm">Demo Tunanetra</Link>
              <Link href="/tunarungu" className="flex items-center justify-center gap-1.5 bg-white text-purple-700 border border-slate-200 py-2.5 rounded-xl text-xs font-bold shadow-sm">Demo Tunarungu</Link>
            </div>
          </div>
        </div>
      </div>
